...
class VehicleService:
    ...
    def _load(self, vehicle_id: int) -> Optional[Vehicle]:
        return (
            self.db.query(Vehicle)
            .options(joinedload(Vehicle.fuel_consumable))
    ...
    def get_all(
        ...
        q = self.db.query(Vehicle).options(joinedload(Vehicle.fuel_consumable))
        if active_only:
    ...
    def create(self, data: VehicleCreate) -> Vehicle:
        ...
        self.db.commit()
        self.db.refresh(obj)
        return self._load(obj.id)

    def update(self, vehicle_id: int, data: VehicleUpdate) -> Optional[Vehicle]:
        obj = self.db.query(Vehicle).filter(Vehicle.id == vehicle_id).first()
        if not obj:
            return None
