...
@router.get("/transformation-types", response_model=ApiResponse[list[TransformationTypeSchema]])
def get_transformation_types(db: Session = Depends(get_db)):
    """Retrieve all transformation types. Auth: any authenticated user."""
    service = SettingsService(db)
    return ApiResponse(data=service.get_transformation_types())


@router.post(
    "/transformation-types", response_model=ApiResponse[TransformationTypeSchema], status_code=201
)
def create_transformation_type(
    data: TransformationTypeCreate, db: Session = Depends(get_db)
):
    """Create a new transformation type."""
    service = SettingsService(db)
    return ApiResponse(
        data=service.create_transformation_type(data),
        message="Transformation type created",
        type="success",
    )


@router.put("/transformation-types/{type_id}", response_model=ApiResponse[TransformationTypeSchema])
def update_transformation_type(
    type_id: int, data: TransformationTypeUpdate, db: Session = Depends(get_db)
):
    ...
    if not result:
        raise HTTPException(
            ...
            detail="Transformation type not found",
        )
    return ApiResponse(data=result, message="Transformation type updated", type="success")


@router.delete("/transformation-types/{type_id}", response_model=ApiResponse[None])
...
@router.post("/wage-types", response_model=ApiResponse[WageTypeSchema], status_code=201)
def create_wage_type(data: WageTypeCreate, db: Session = Depends(get_db)):
    """Create a new wage type."""
    service = SettingsService(db)
    return ApiResponse(
        data=service.create_wage_type(data),
        message="Wage type created",
        type="success",
    )


@router.put("/wage-types/{type_id}", response_model=ApiResponse[WageTypeSchema])
def update_wage_type(type_id: int, data: WageTypeUpdate, db: Session = Depends(get_db)):
    ...
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Wage type not found"
        )
    return ApiResponse(data=result, message="Wage type updated", type="success")


@router.delete("/wage-types/{type_id}", response_model=ApiResponse[None])
def delete_wage_type(type_id: int, db: Session = Depends(get_db)):
    ...
    return ApiResponse(data=None, message="Wage type deleted", type="success")
...
@router.get("/batch-stages", response_model=ApiResponse[list[BatchStageSchema]])
def get_batch_stages(db: Session = Depends(get_db)):
    """Retrieve all batch stages. Auth: any authenticated user."""
    service = SettingsService(db)
    return ApiResponse(data=service.get_batch_stages())


@router.post("/batch-stages", response_model=ApiResponse[BatchStageSchema], status_code=201)
def create_batch_stage(data: BatchStageCreate, db: Session = Depends(get_db)):
    """Create a new batch stage."""
    service = SettingsService(db)
    return ApiResponse(
        data=service.create_batch_stage(data),
        message="Batch stage created",
        type="success",
    )


@router.put("/batch-stages/reorder", response_model=ApiResponse[list[BatchStageSchema]])
def reorder_batch_stages(
    items: list[BatchStageReorderItem], db: Session = Depends(get_db)
):
    ...
    return ApiResponse(data=result, message="Batch stages reordered", type="success")


@router.put("/batch-stages/{stage_id}", response_model=ApiResponse[BatchStageSchema])
def update_batch_stage(
    stage_id: int, data: BatchStageUpdate, db: Session = Depends(get_db)
):
    ...
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Batch stage not found"
        )
    return ApiResponse(data=result, message="Batch stage updated", type="success")


@router.delete("/batch-stages/{stage_id}", response_model=ApiResponse[None])
def delete_batch_stage(stage_id: int, db: Session = Depends(get_db)):
    ...
    return ApiResponse(data=None, message="Batch stage deleted", type="success")
...
@router.get("/expense-categories", response_model=ApiResponse[list[ExpenseCategorySchema]])
def get_expense_categories(db: Session = Depends(get_db)):
    """Retrieve all expense categories. Auth: any authenticated user."""
    service = SettingsService(db)
    return ApiResponse(data=service.get_expense_categories())


@router.post(
    "/expense-categories", response_model=ApiResponse[ExpenseCategorySchema], status_code=201
)
def create_expense_category(data: ExpenseCategoryCreate, db: Session = Depends(get_db)):
    """Create a new expense category."""
    service = SettingsService(db)
    return ApiResponse(
        data=service.create_expense_category(data),
        message="Expense category created",
        type="success",
    )


@router.put("/expense-categories/{cat_id}", response_model=ApiResponse[ExpenseCategorySchema])
def update_expense_category(
    cat_id: int, data: ExpenseCategoryUpdate, db: Session = Depends(get_db)
):
    ...
    return ApiResponse(data=result, message="Expense category updated", type="success")


@router.delete("/expense-categories/{cat_id}", response_model=ApiResponse[None])
def delete_expense_category(cat_id: int, db: Session = Depends(get_db)):
    ...
    if result == "system_category":
        raise HTTPException(
            status_code=400, detail="Cannot delete system expense category"
        )
    return ApiResponse(data=None, message="Expense category deleted", type="success")
...
@router.get("/app-config/{key}", response_model=ApiResponse[AppConfigSchema])
...
@router.put("/app-config/{key}", response_model=ApiResponse[AppConfigSchema])
def set_app_config(key: str, payload: AppConfigUpdate, db: Session = Depends(get_db)):
    ...
    return ApiResponse(data=obj, message="Configuration updated", type="success")
