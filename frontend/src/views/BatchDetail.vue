<template>
    <div class="batch-detail-page" v-if="batch">
            ...
            <div class="header-left">
                ...
                <div>
                    <div class="header-title-row">
                        ...
                        <span
                            class="stage-badge"
                            :class="stageBadgeClass(batch.stage_name)"
                        >
                            {{ batch.stage_name || "Unknown Stage" }}
                        </span>
                        <span
                            class="status-dot"
                            :class="batch.is_depleted ? 'depleted' : 'active'"
                        >
                            <span class="dot"></span
                    ...
                    <p class="page-subtitle">
                        ...
                        <span v-if="batch.plantation_name">
                            · {{ batch.plantation_name }}</span
                        >
                    </p>
                </div>
            </div>
        ...
        <div
            v-if="activeTab === 'tree'"
            class="content-panel animate-fade-in-up animate-delay-2"
        >
            <div class="panel-header"><span>Batch Genealogy</span></div>
            <div class="panel-body">
                <div v-if="genealogyLoading" class="empty-state">
                    <i class="bi bi-hourglass-split"></i>
                    <p>Loading tree...</p>
                </div>
                <div v-else-if="!genealogy" class="empty-state">
                ...
                <!-- <div v-else class="tree-container">
                     Parents (ancestors)
                    <div
                        v-if="genealogy.parents && genealogy.parents.length"
                        class="tree-section"
                    >
                        {{ genealogy.parents }}
                        ...
                        <div class="tree-parents">
                            <BatchTreeNode
                                v-for="parent in genealogy.parents"
                                :key="parent.batch_id"
                                :node="parent"
                                :currentId="batch.id"
                                direction="up"
                    ...
                    <div class="tree-current-node">
                        <div class="tree-node-content current">
                            <span class="tree-code">{{
                                genealogy.batch_code
                            }}</span>
                            <span
                                v-if="genealogy.stage_name"
                                class="tree-stage"
                                >{{ genealogy.stage_name }}</span
                            >
                            <span class="tree-weight"
                                >{{
                                    Number(
                                        genealogy.remaining_weight_kg,
                                    ).toFixed(2)
                                }}
                                kg</span
                            >
                            <span
                                v-if="genealogy.is_depleted"
                                class="tree-depleted"
                                >Depleted</span
                            >
                        </div>
                    ...
                    <div
                        v-if="genealogy.children && genealogy.children.length"
                        class="tree-section"
                    >
                        <div class="tree-connector">
                            <i class="bi bi-arrow-down"></i>
                        ...
                        <div class="tree-children-section">
                            <BatchTreeNode
                                v-for="child in genealogy.children"
                                :key="child.batch_id"
                                :node="child"
                                :currentId="batch.id"
                                direction="down"
        ...
        >
            ...
            <div class="panel-body">
                <div class="timeline">
                    <div
                        v-if="relatedTransformations.length === 0"
                    ...
                    <div
                        class="timeline-item"
                        v-for="t in relatedTransformations"
                        :key="t.id"
                    >
                        <div
                            class="timeline-icon"
                            :class="t.role === 'output' ? 'harvest' : 'process'"
                        >
                            <i
                                class="bi"
                                :class="
                        ...
                        <div class="timeline-content">
                            <div class="timeline-title">
                                {{ t.type_name }} — T-{{ t.id }}
                                <span class="role-tag" :class="t.role">{{
                                    t.role === "output"
                                        ? "Created this batch"
                                        : "Consumed this batch"
                                }}</span>
                            ...
                            <div class="timeline-meta">
                                ...
                                <span v-if="t.to_date">
                                    · Completed
                                    {{ formatDate(t.to_date) }}</span
                                >
                                <span v-else class="in-progress-tag">
                                    · In Progress</span
                                >
                            </div>
                        </div>
            ...
            <div class="panel-body">
                <div class="details-grid">
                    <div class="form-group full-width">
                        ...
                        <select
                            v-model="editForm.plantation_id"
                            class="form-control form-select"
                            :disabled="!isAdmin"
                        >
                            <option :value="null">— None —</option>
                            <option
                                v-for="p in plantations"
                                :key="p.id"
                                :value="p.id"
                            >
                                {{ p.name }}
                    ...
                    <div class="meta-info">
                        ...
                        <span
                            >Initial weight:
                            {{ formatKg(batch.initial_weight_kg) }} kg</span
                        >
                        <span
                            >Remaining weight:
                            {{ formatKg(batch.remaining_weight_kg) }} kg</span
                        >
                    </div>
                </div>
    ...
    <div v-else-if="loadError" class="empty-state page-empty">
        ...
        <p>{{ loadError }}</p>
        <button class="btn-primary" @click="$router.push({ name: 'batches' })">
            Back to Batches
        </button>
    </div>
            ...
            <div class="modal-content agri-modal">
                ...
                <div class="agri-modal-body">
                    <p class="confirm-text">
                        Delete batch <strong>{{ batch?.batch_code }}</strong
...
import BatchTreeNode from "@/components/BatchTreeNode.vue";
...
async function fetchRelatedTransformations() {
    try {
        const res = await api.get(
            `/batches/${route.params.id}/transformations`,
        );
...
async function deleteBatch() {
    ...
    try {
        ...
        if (err.response?.status === 409) {
            deleteError.value =
                "Cannot delete: batch is referenced by a transformation.";
        } else {
            deleteError.value =
...
async function loadBatchData() {
    ...
    relatedTransformations.value = [];
    loadError.value = "";
    await fetchBatch();
    fetchGenealogy();
    fetchRelatedTransformations();
    fetchPlantations();
}

// Re-fetch when navigating between batches (same component, different param)
watch(
    () => route.params.id,
    (newId, oldId) => {
        if (newId && newId !== oldId) loadBatchData();
    },
);
